...
function doBalance(message, tipper) {
  spk.getBalance(tipper, 1, function(err, balance) {
    if (err) {
      message.reply('Error getting SparksPay (SPK) balance.').then(message => message.delete(10000));
    } else {
...
function doDeposit(message, tipper) {
  getAddress(tipper, function(err, address) {
    if (err) {
      message.reply('Error getting your SparksPay (SPK) deposit address.').then(message => message.delete(10000));
    } else {
    message.channel.send({ embed: {
...
function txLink(txId) {
  return 'http://explorer.sparkspay.io/tx/' + txId;
}

function addyLink(address) {
  return 'http://explorer.sparkspay.io/address/' + address;
}